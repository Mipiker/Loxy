...
class TextInput extends Button {
	...
	void update(Input input, Vector2f windowSize, Vector2f pos) {
		...
		frame.setSize(new Vector2i(width, font.getSize().y));
		frame.setPos(new Vector2f(pos));
		...
		if (selected) {
			...
			for (Character c : input.getKeyCharPressed())
				if (text.length() < maxChar && textComponent.getSize().x + font.getCharTextureWidth(c).getWidth()
						* textComponent.getTransformation().getScale(new Vector3f()).x < width)
					setText(text + c);
		}
		frame.setFrameColor(selected ? new Vector4f(0.4f, 0.69f, 0.20f, 1) : new Vector4f(0.5f, 0.5f, 0.5f, 1));
		textComponent.setColor(selected ? new Vector4f(1, 1, 1, 1) : new Vector4f(0.5f, 0.5f, 0.5f, 1));
	}

	void resetFrameComponent() {
		unShow();
		frame = new HudFrameComponent(new Vector2i(width, font.getSize().y));
